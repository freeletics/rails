  ...
  module Command
    class CredentialsCommand < Rails::Command::Base # :nodoc:
      ...
      class_option :environment, aliases: "-e", type: :string,
        desc: "Takes credentials from config/credentials/:environment.yml.enc encrypted by config/credentials/:environment.key key"
      ...
      private
        ...
        def missing_encrypted_message(key:, key_path:, file_path:)
          if key.nil?
            ...
            "File '#{file_path}' does not exist. Use `rails credentials:edit #{file_path}` to change that."
          end
        end
    end
  end
